...
export function VideoBlock({ componentId, url }: { componentId: string; url?: string }) {
  ...
  useEffect(() => {
    if (seeking) {
      updateMyPresence({ state: "seeking" } as any);
      return;
    }

    updateMyPresence({ state: playing ? "playing" : "paused" } as any);
  ...
  return (
    <div className={cn(styles.videoPlayer, "bg-white h-full w-full flex flex-col")}>
      <div className={styles.playerWrapper} ref={playerWrapper}>
        ...
        >
          <ClientSideSuspense fallback={null}>
            <ReactPlayer
              ...
              onEnded={handleEnded}
              url={url || "michael-james-16296843-720p.mp4"}
        ...
        <div className={styles.sliderAndComments}>
          ...
          <div className={styles.sliderComments}>
            <ThreadsTimeline componentId={componentId} />
          </div>
