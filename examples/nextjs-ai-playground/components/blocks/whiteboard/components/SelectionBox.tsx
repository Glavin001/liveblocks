...
const SelectionBox = memo(
  ({ onResizeHandlePointerDown }: SelectionBoxProps) => {
    ...
    const soleLayerId = useSelf((me) => {
      const selection = me.presence.whiteboardSelection;
      if (selection?.componentId !== componentId) return null;
      return selection.layerIds.length === 1 ? selection.layerIds[0] : null;
    });

    const isShowingHandles = useStorage(
      (root) =>
        soleLayerId && root.whiteboardData.get(componentId)?.layers.get(soleLayerId)?.type !== LayerType.Path
    );
